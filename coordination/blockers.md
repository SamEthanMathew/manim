...
- **B-010** — `modal.Sandbox` handle shuts down immediately after creation. `modal.exception.NotFoundError: Modal Sandbox with container ID ta-... not found.` raised on first `.exec()` call. Reproduced 2026-05-13 with `modal` 1.4.2 on workers/lib/sandbox.py. Workaround: `RENDER_USE_SANDBOX=0` (default) uses `subprocess.run` inside the worker container, which retains the AST safety gate but loses network isolation. Long-term fix: investigate Modal SDK update or `app=` argument, then flip the env var back to "1".
